...
function enemyListTick() {
    for (let i = enemies.length - 1; i >= 0; i--) {
        ...
        // Boss 进场时停止场景滚动（原版用链表尾节点判断，这里用最后添加的敌人）
        if (!e._hasNext && inScreen) {
            moveScene = 0;
        }
...
async function loadAndStartLevel(levelNum) {
    ...
    if (!levelData) { levelLoading = false; return; }
    ...
    // 标记最后一个敌人（Boss），用于场景滚动停止判定（对应原版链表的 !Current->Next）
    for (let i = 0; i < enemies.length; i++) {
        enemies[i]._hasNext = (i < enemies.length - 1);
    }
    levelLoading = false;
}
